...
var Api = {
  ...
  req: async function (method, path, body) {
    ...
    if (!res.ok) {
      var apiErr = new Error(data.message || 'Request failed (' + res.status + ')');
      apiErr.status = res.status;      // lets callers branch on e.g. 409 without changing e.message handling anywhere else
      apiErr.data = data;
      throw apiErr;
    }
    return data;
...
/* Escapes free-text that a user typed (not app-authored strings) before
   it's inserted via innerHTML — used for admin-authored rejection
   reasons, which are the one place in this file where arbitrary
   multi-sentence text from one role is shown inside another role's UI. */
...
/* Prompts for a required rejection reason before rejecting an exam.
   The teacher sees this reason on their end, so a real explanation (not
   just "rejected") is enforced both here and again server-side. */
function promptReject(examTitle, onSubmit) {
  var ov = document.createElement('div'); ov.className = 'confirm-overlay';
  ov.innerHTML =
    '<div class="confirm-box" style="text-align:left">'
    + '<h4 style="text-align:center">❌ Reject Exam</h4>'
    + '<p id="rejExamName" style="text-align:center;font-weight:600"></p>'
    + '<p style="text-align:center">Please tell the teacher why, so they can fix it and resubmit.</p>'
    + '<textarea id="rejReasonInput" rows="4" placeholder="e.g. Question 7 has two correct options — please review."'
    +   ' style="width:100%;border-radius:8px;padding:10px;border:1.5px solid var(--border);font-family:var(--font-body);font-size:.88rem;background:var(--surface-2);color:var(--text);resize:vertical;margin-bottom:8px"></textarea>'
    + '<div id="rejReasonError" style="color:#dc2626;font-size:.78rem;margin-bottom:10px;display:none"></div>'
    + '<div class="confirm-actions"><button class="btn-secondary" id="rejCancel">Cancel</button>'
    + '<button class="btn-danger" id="rejOk">Reject Exam</button></div></div>';
  document.body.appendChild(ov);
  ov.querySelector('#rejExamName').textContent = examTitle;   // textContent, not innerHTML — exam titles are user-entered
  var input = ov.querySelector('#rejReasonInput');
  var errEl = ov.querySelector('#rejReasonError');
  input.focus();
  ov.querySelector('#rejOk').addEventListener('click', function () {
    var reason = input.value.trim();
    if (!reason) {
      errEl.textContent = 'Please enter a reason before rejecting.';
  ...
  ov.querySelector('#rejCancel').addEventListener('click', function () { ov.remove(); });
}
...
var TITLES = {
  ...
  'missing-submissions': 'Missing Submissions',
...
function renderSection(name) {
  ...
  if (s.role === 'teacher') {
    ...
    if (name === 'missing-submissions') renderTeacherMissingSubmissions(s);
...
function rejectExam(id) {
  var e = _examCache.find(function (x) { return String(x._id) === String(id); });
  var title = e ? e.title : 'this exam';
  promptReject(title, async function (reason) {
    try {
      await Api.put('/exams/' + id + '/status', { status: 'rejected', reason: reason });
      showToast('Exam rejected — the teacher has been notified with your reason.', 'error');
      renderExamQueue(_qFilter);
      renderAdminOverview();
    } catch (err) { showToast('Error: ' + err.message, 'error'); }
  });
}
...
async function renderTeacherMyExams(s) {
  ...
  try {
    ...
    list.innerHTML = myExams.map(function (e) {
      var qc = (e.questions || []).length + (e.theoryQuestions || []).length;
      var rejBanner = (e.status === 'rejected' && e.rejectionReason)
        ? '<div class="queue-card-reason">🛈 Admin\u2019s reason: ' + escHtml(e.rejectionReason) + '</div>'
        : '';
      return '<div class="queue-card"><div class="queue-card-info"><div class="queue-card-title">' + e.title + '</div><div class="queue-card-meta"><span>📚 ' + e.subject + '</span><span>🏫 ' + e.targetClass + '</span><span>❓ ' + qc + ' Qs</span><span>⏱ ' + e.duration + 'min</span><span>🏆 ' + (e.totalMarks || 0) + ' marks</span></div>' + rejBanner + '</div><div class="queue-card-actions"><span class="badge badge-' + e.status + '">' + cap(e.status) + '</span><button class="btn-secondary btn-sm" onclick="ecEditExam(\'' + sid(e._id) + '\')">✏️ Edit</button></div></div>';
    }).join('');
  } catch (err) { list.innerHTML = emptyHTML('⚠️', err.message); }
}
...
/* MISSING SUBMISSIONS — which students in a class haven't submitted a
   ...
   GET /exams (this teacher's own), GET /students/by-class (roster),
   GET /results/exam/:examId (who has submitted) — no backend changes
...
async function renderTeacherMissingSubmissions(s) {
  ...
  var picker  = $('msExamPicker');
  if (!picker) return;
  if (summary) summary.innerHTML = '';
  if (list) list.innerHTML = loadingHTML('Loading your exams…');
  try {
    var exams = await Api.get('/exams');
    msExamsCache = exams.filter(function (e) {
      return (e.createdByName === s.name || e.createdBy === s.username) && e.status === 'approved';
    });
    msPopulateExamPicker();
    if (list) {
      list.innerHTML = msExamsCache.length === 0
        ? emptyHTML('📭', 'You have no approved exams yet — missing submissions can only be tracked once an exam is approved and visible to students.')
        : emptyHTML('👆', 'Select an exam above to see who hasn\'t submitted yet.');
    }
  } catch (err) {
...
function msPopulateExamPicker() {
  var termFilter = ($('msTermFilter') || {}).value || '';
  var picker = $('msExamPicker'); if (!picker) return;
  var prevValue = picker.value;
  var filtered = termFilter ? msExamsCache.filter(function (e) { return e.term === termFilter; }) : msExamsCache;

  picker.innerHTML = '<option value="">Select an exam…</option>'
    + filtered.map(function (e) {
        return '<option value="' + sid(e._id) + '">' + e.subject + ' — ' + e.targetClass + ' — ' + (e.term || 'No term') + (e.session ? ' ' + e.session : '') + '</option>';
      }).join('');

  var stillValid = filtered.some(function (e) { return sid(e._id) === prevValue; });
  if (stillValid) {
...
async function loadMissingSubmissions() {
  var picker  = $('msExamPicker');
  var examId  = picker ? picker.value : '';
  var summary = $('missingSubmissionsSummary');
  var list    = $('missingSubmissionsList');
  ...
  var exam = msExamsCache.find(function (e) { return sid(e._id) === examId; });
  if (!exam) return;
  if (summary) summary.innerHTML = '';
  if (list) list.innerHTML = loadingHTML('Checking submissions…');

  try {
    var roster  = await Api.get('/students/by-class?class=' + encodeURIComponent(exam.targetClass));
    var results = await Api.get('/results/exam/' + examId);

    var submittedIds = {};
    results.forEach(function (r) { submittedIds[sid(r.student)] = true; });
    ...
    if (list) {
      if (roster.length === 0) {
        list.innerHTML = emptyHTML('🏫', 'No approved students found in ' + exam.targetClass + '.');
      } else if (missing.length === 0) {
        list.innerHTML = emptyHTML('✅', 'Everyone in ' + exam.targetClass + ' has submitted this exam.');
      } else {
        list.innerHTML = '<div class="table-wrap"><table class="data-table"><thead><tr><th>#</th><th>Name</th><th>Username</th><th>Class</th></tr></thead><tbody>'
          + missing.map(function (st, i) {
              return '<tr><td>' + (i + 1) + '</td><td>' + (st.firstName || '') + ' ' + (st.lastName || '') + '</td><td>' + (st.username || '') + '</td><td>' + (st.class || '') + '</td></tr>';
            }).join('')
          + '</tbody></table></div>';
      }
    }
...
/* Full score editor: shows the auto-graded objective breakdown (read-only,
   for context) with an editable override, plus editable theory marks if
   the exam has a theory section. Used both from the Marking screen (before
   a result is submitted to Admin) and from Student Submissions (any time
   afterward) — the same modal and save logic either way. */
async function openScoreEditor(resultId) {
  try {
    var results = await Api.get('/results');
    var r = results.find(function (x) { return String(x._id) === String(resultId); });
    if (!r) { showToast('Result not found.', 'error'); return; }

    var hasObj    = (r.objBreakdown   || []).length > 0;
    var hasTheory = (r.theoryAnswers  || []).length > 0;

    var html = '<div class="theory-marking">'
      + '<h4 style="font-family:var(--font-display);margin-bottom:4px">Edit Score</h4>'
      + '<p style="font-size:.83rem;color:var(--text-2);margin-bottom:16px">Student: <strong>' + (r.studentName || '') + '</strong> — ' + (r.examTitle || '') + '</p>';

    if (hasObj) {
      html += '<div class="preview-section-title">SECTION A — OBJECTIVE (auto-graded)</div>'
        + '<div class="table-wrap" style="margin-bottom:12px"><table class="data-table" style="font-size:.8rem"><thead><tr>'
        + '<th>#</th><th>Question</th><th>Student\'s Answer</th><th>Correct Answer</th><th>Result</th>'
        + '</tr></thead><tbody>'
        + r.objBreakdown.map(function (b, i) {
            return '<tr><td>' + (i + 1) + '</td><td>' + (b.text || '') + '</td><td>' + (b.studentAns || '') + '</td><td>' + (b.correctAns || '') + '</td><td>' + (b.correct ? '✅' : '❌') + '</td></tr>';
          }).join('')
        + '</tbody></table></div>'
        + '<div class="marks-row"><label>Objective Score Override:</label>'
        + '<input type="number" class="marks-input" id="obj_score_override" value="' + (r.objScore || 0) + '" min="0" max="' + (r.objTotal || 0) + '" step="0.5"/>'
        + '<span class="marks-note">out of ' + (r.objTotal || 0) + ' (auto-graded: ' + (r.objScore || 0) + ')</span></div>';
    }

    if (hasTheory) {
      html += '<div class="preview-section-title" style="margin-top:20px">SECTION B — THEORY</div>';
    ...
    if (!hasObj && !hasTheory) {
      html += '<p style="color:var(--text-3);font-size:.85rem">This exam has no scoreable sections.</p>';
    }

    html += '</div>';
    $('examPreviewContent').innerHTML = html;
    $('examPreviewActions').innerHTML = '<button class="btn-secondary" onclick="closeExamPreview()">Cancel</button>'
      + ((hasObj || hasTheory) ? '<button class="btn-primary" onclick="saveScoreEditor(\'' + resultId + '\')">💾 Save Score</button>' : '');
    $('examPreviewOverlay').classList.remove('hidden');
  } catch (err) { showToast('Error: ' + err.message, 'error'); }
}

/* Kept so the Marking screen's existing "✍️ Mark Theory" button (which
   calls this name) keeps working unchanged — it now opens the fuller
   score editor, which is a strict superset of what it did before. */
function openTheoryMarking(resultId) { return openScoreEditor(resultId); }

async function saveScoreEditor(resultId) {
  try {
    var results = await Api.get('/results');
    var r = results.find(function (x) { return String(x._id) === String(resultId); });
    if (!r) { showToast('Result not found.', 'error'); return; }

    var objTotal   = r.objTotal || 0;
    var objInput   = document.getElementById('obj_score_override');
    var newObjScore = objInput ? Math.max(0, Math.min(objTotal, parseFloat(objInput.value) || 0)) : (r.objScore || 0);

    var theoryAnswers = r.theoryAnswers || [];
    var totalTheory = 0;
    var updatedAnswers = theoryAnswers.map(function (a, i) {
      var el = document.getElementById('theory_mark_' + i);
      var v = el ? Math.max(0, Math.min(a.maxMarks || 0, parseFloat(el.value) || 0)) : (a.marksAwarded || 0);
      totalTheory += v;
      return Object.assign({}, a, { marksAwarded: v });
    });
    var newTheoryScore = theoryAnswers.length > 0 ? totalTheory : r.theoryScore;

    var grandTotal = r.grandTotal || (objTotal + (r.theoryTotal || 0));
    var newTotal   = newObjScore + (newTheoryScore || 0);
    var percent    = grandTotal > 0 ? Math.round((newTotal / grandTotal) * 100) : 0;
    var grade      = calcGrade(percent);

    await Api.put('/results/' + resultId, {
      objScore: newObjScore,
      theoryAnswers: updatedAnswers,
    ...
    closeExamPreview();
    showToast('Score updated! New total: ' + newTotal + '/' + grandTotal, 'success');
    refreshCurrentTeacherView();
  } catch (err) { showToast('Error: ' + err.message, 'error'); }
}

/* Re-renders whichever teacher section is currently visible after a
   score edit, instead of guessing or refreshing every section. */
function refreshCurrentTeacherView() {
  var s = Session.get(); if (!s) return;
  var subSec  = document.getElementById('section-submissions');
  var markSec = document.getElementById('section-marking');
  if (subSec  && subSec.classList.contains('active')  && typeof renderTeacherSubmissions === 'function') renderTeacherSubmissions(s);
  if (markSec && markSec.classList.contains('active') && typeof renderTeacherMarking === 'function')      renderTeacherMarking(s);
}
