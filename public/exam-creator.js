...
var EC = {
  ...
  autosaveTimer  : null,
  activeTab      : 'objective',
  editingExamId  : null,  // null = creating a new exam; set = editing an existing one
  editingExamStatus: null,          // status of the exam being edited, at load time ('pending'|'approved'|'rejected')
  editingExamRejectionReason: ''    // admin's reason, shown while fixing a rejected exam
};
...
function initCreateExam() {
  if (EC.editingExamId) {
    /* Edit mode: form fields and EC.objQuestions/theoryQuestions were
       already populated by ecEditExam() before this section was shown.
       Loading the autosave draft here would overwrite the exam being
       edited with an unrelated leftover draft, so skip it entirely. */
    ecRenderAll();
    ecShowPanel(EC.activeTab);
    ecUpdateSummary();
    ecUpdateCounts();
...
function clearExamForm() {
  ecConfirm('Clear Draft', 'This will erase your current draft and all questions. Are you sure?', function() {
    localStorage.removeItem(EC.DRAFT_KEY);
    EC.editingExamId   = null;
    EC.editingExamStatus = null;
    EC.editingExamRejectionReason = '';
    EC.objQuestions    = [ecNewObj()];
    EC.theoryQuestions = [ecNewTheory()];
    ['examTitle','examSubject','examClass','examDuration','examSession','examTerm','examExpiry'].forEach(function(id){ ecSetVal(id,''); });
    ecRenderAll(); ecUpdateSummary(); ecUpdateCounts(); ecSetStatus('saved'); ecUpdateEditModeUI();
    ecToast('Draft cleared.', 'info');
  });
}
...
/* ══════════════════════════════════════════════════════════
   ...
   Loads a previously-created exam (any status) into the same
   creator UI used for new exams. Fetches fresh from the server
   rather than trusting a possibly-stale local list cache.
   ══════════════════════════════════════════════════════════ */
async function ecEditExam(examId) {
  try {
    if (typeof Api === 'undefined') { ecToast('API not available. Make sure server is running.', 'error'); return; }
    var exam = await Api.get('/exams/' + examId);

    EC.editingExamId = exam._id;
    EC.editingExamStatus = exam.status || null;
    EC.editingExamRejectionReason = exam.rejectionReason || '';

    ecSetVal('examTitle',    exam.title       || '');
    ecSetVal('examSubject',  exam.subject     || '');
    ecSetVal('examClass',    exam.targetClass || '');
    ecSetVal('examDuration', exam.duration    || '');
    ecSetVal('examSession',  exam.session     || '');
    ecSetVal('examTerm',     exam.term        || '');
    ecSetVal('examExpiry',   ecToLocalDatetimeValue(exam.expiresAt));

    EC.objQuestions = (exam.questions || []).map(function (q) {
      var nq = ecNewObj();
      nq.text    = q.text || '';
      nq.options = Array.isArray(q.options) ? q.options.slice(0, 4) : [];
    ...
    EC.theoryQuestions = (exam.theoryQuestions || []).map(function (q) {
      var nq = ecNewTheory();
      nq.text  = q.text  || '';
      nq.guide = q.guide || '';
      nq.marks = q.marks || 5;
    ...
    if (typeof showSection === 'function') showSection('create-exam');
  } catch (e) {
    ecToast('Could not load exam for editing: ' + e.message, 'error');
  }
}

function ecCancelEdit() {
  ecConfirm('Cancel Edit', 'Discard your changes and go back to My Exams?', function () {
    EC.editingExamId    = null;
    EC.editingExamStatus = null;
    EC.editingExamRejectionReason = '';
    EC.objQuestions     = [ecNewObj()];
...
/* Converts a stored ISO date string into the "YYYY-MM-DDTHH:mm" format
   a <input type="datetime-local"> expects, using LOCAL time components
   (not UTC) — matching how the value was originally entered. */
function ecToLocalDatetimeValue(iso) {
  if (!iso) return '';
  var d = new Date(iso);
...
/* Swaps labels/visibility between "create" and "edit" modes, and shows
   the admin's rejection reason (if any) while fixing a rejected exam.
   Called whenever the create-exam section is (re)shown. */
function ecUpdateEditModeUI() {
  var editing     = !!EC.editingExamId;
  var isRejection = editing && EC.editingExamStatus === 'rejected';
  var titleEl   = document.getElementById('ceTopbarTitle');
  var submitBtn = document.getElementById('ceSubmitBtn');
  var submitBtn2= document.getElementById('ceSubmitBtnModal');
  var cancelBtn = document.getElementById('ceCancelEditBtn');
  var clearBtn  = document.getElementById('ceClearDraftBtn');
  var rejBanner = document.getElementById('ceRejectionBanner');

  if (titleEl)    titleEl.textContent    = isRejection ? '🔁 Fix Rejected Exam' : (editing ? '✏️ Edit Examination' : '➕ Create New Examination');
  if (submitBtn)  submitBtn.textContent  = isRejection ? '🔁 Fix & Resubmit for Approval' : (editing ? '💾 Save Changes' : 'Submit to Admin for Approval 📤');
  if (submitBtn2) submitBtn2.textContent = isRejection ? '🔁 Fix & Resubmit' : (editing ? '💾 Save Changes' : 'Submit to Admin 📤');
  if (cancelBtn)  cancelBtn.classList.toggle('hidden', !editing);
  if (clearBtn)   clearBtn.classList.toggle('hidden', editing);

  if (rejBanner) {
    if (isRejection) {
      var reasonText = EC.editingExamRejectionReason
        ? EC.editingExamRejectionReason
        : 'No reason was recorded.';
      rejBanner.textContent = '🛈 This exam was rejected by Admin. Reason: ' + reasonText + ' Fix the issue above, then click "Fix & Resubmit for Approval".';
      rejBanner.classList.remove('hidden');
    } else {
      rejBanner.classList.add('hidden');
      rejBanner.textContent = '';
    }
  }
}
...
function ecToggleObjPaste() {
  ...
  // Closing the box (Cancel, or after a fully clean parse) clears any
  // stale report so the next time it's opened starts fresh.
  if (box.classList.contains('hidden')) {
    var report = document.getElementById('objPasteReport');
...
function ecParsePastedObjective() {
  ...
  /* Accept ".", ")", ":" or "-" after the question number — Word/Docs
     exports and manually-typed lists both show up with any of these,
     and a format mismatch here is the #1 cause of whole questions
     silently merging into a neighboring block and failing together. */
  var qStart = /^\s*(?:Q(?:uestion)?\s*)?\d+[\.\)\:\-]\s*/i;
  var optLine = /^\s*\(?([A-Da-d])\)?[\.\):]\s*(.+)$/;
  var ansLine = /^\s*(?:correct\s*)?ans(?:wer)?\s*[:\-]?\s*\(?([A-Da-d0-9])\)?/i;
  var letterIndex = { A:0, B:1, C:2, D:3 };
  ...
  blocks.forEach(function (blockLines) {
    ...
    var reason = null;
    if (!text)                             reason = 'No question text detected.';
    else if (options.length < 2)           reason = 'Fewer than 2 options detected (' + options.length + ' found).';
    else if (answerIdx < 0)                reason = 'No "Answer: X" line detected.';
    else if (answerIdx > options.length - 1) reason = 'Answer letter doesn\u2019t match any option (' + options.length + ' option(s) found).';

    if (reason) {
      failedBlocks.push({ text: blockLines.join('\n').trim(), reason: reason });
      return;
    }
  ...
  if (parsed.length === 0 && failedBlocks.length === 0) {
    ecToast('Could not parse any questions — check the format shown above and try again.', 'error');
    return;
  }
  ...
  ecRenderPasteReport(parsed.length, failedBlocks);

  if (failedBlocks.length === 0) {
    ecToast('Added ' + parsed.length + ' question(s) from paste.', 'success');
    area.value = '';
    ecToggleObjPaste();
  } else {
    /* Leave the box open and pre-fill the textarea with ONLY the failed
       blocks — already-successful questions are inserted immediately and
       won't be re-added, so fixing formatting here and re-parsing is safe. */
    area.value = failedBlocks.map(function (b) { return b.text; }).join('\n\n');
    ecToast(parsed.length + ' added, ' + failedBlocks.length + ' need fixing \u2014 see report below.', 'info');
  }
}

/* Shows exactly what happened after a paste-parse: a success count, and
   for every block that failed, its original text plus the specific
   reason — so nothing is ever lost silently. */
function ecRenderPasteReport(addedCount, failedBlocks) {
  var report = document.getElementById('objPasteReport');
  if (!report) return;

  if (addedCount === 0 && failedBlocks.length === 0) {
    report.innerHTML = ''; report.classList.add('hidden'); return;
  }

  var html = '<div style="margin-top:12px;padding:10px 12px;border-radius:8px;background:#f0fdf4;border:1px solid #bbf7d0;color:#166534;font-size:13px;font-weight:600;">'
    + '\u2705 ' + addedCount + ' question(s) added to your exam.</div>';

  if (failedBlocks.length > 0) {
    html += '<div style="margin-top:10px;padding:12px;border-radius:8px;background:#fef2f2;border:1px solid #fecaca;">'
      + '<div style="color:#991b1b;font-weight:700;font-size:13px;margin-bottom:8px;">\u26a0\ufe0f ' + failedBlocks.length + ' question(s) could NOT be added \u2014 fix the formatting below and click "Parse &amp; Insert" again:</div>'
      + failedBlocks.map(function (b) {
          return '<div style="margin-bottom:8px;padding:8px;background:#fff;border-radius:6px;border:1px solid #fecaca;">'
            + '<div style="font-size:12px;color:#991b1b;font-weight:600;margin-bottom:4px;">Reason: ' + ecEsc(b.reason) + '</div>'
            + '<pre style="white-space:pre-wrap;font-family:monospace;font-size:12px;color:#444;margin:0;">' + ecEsc(b.text) + '</pre>'
            + '</div>';
        }).join('')
      + '<div style="font-size:12px;color:#7f1d1d;">These have been copied back into the paste box above so you don\u2019t have to retype them.</div>'
      + '</div>';
  }
...
async function submitExamToAdmin(){
  ...
  /* Submit to MongoDB via API */
  if(!schoolId) return ecShowErr(errEl,'School ID not found. Please log out and log in again.');
  var isEditing = !!EC.editingExamId;
  var submitPayload = {
    schoolId:schoolId,
    title:title, subject:subject, targetClass:cls, duration:duration,
    session:session, term:term, expiresAt:expiresAt, totalMarks:oT+tT, objMarks:oT, theoryMarks:tT,
    questions:EC.objQuestions.map(function(q){return{type:'objective',text:q.text,options:q.options,answer:q.answer,marks:q.marks};}),
    theoryQuestions:EC.theoryQuestions.map(function(q){return{type:'theory',text:q.text,guide:q.guide,marks:q.marks};}),
    createdBy:tUser, createdByName:tName
  };
  if (!isEditing) submitPayload.status = 'pending';   // edits keep the exam's current status server-side

  try{
    if(typeof Api==='undefined'){ ecShowErr(errEl,'API not available. Make sure server is running.'); return; }
    if (isEditing) {
      await Api.put('/exams/' + EC.editingExamId, submitPayload);
    } else {
      /* The server auto-creates the admin "new exam pending" notification
         inside POST /exams — no second client call needed. */
      await Api.post('/exams', submitPayload);
    ...
    /* Server blocks direct edits once students have already submitted
       answers, to protect grading integrity — offer to duplicate as a
       new exam instead of just showing an error and losing the work. */
    if (isEditing && e.status === 409) {
      ecShowErr(errEl, e.message);
      ecShowDuplicateOffer(errEl);
      return;
    }
    return ecShowErr(errEl,'Submission failed: '+e.message);
  }

  var wasResubmission = isEditing && EC.editingExamStatus === 'rejected';
  ecToast(wasResubmission ? '✅ Fixed and resubmitted for admin approval!' : (isEditing ? '✅ Exam updated!' : '✅ Exam submitted to Admin for approval!'), 'success');
  localStorage.removeItem(EC.DRAFT_KEY);
  EC.editingExamId = null;
  EC.editingExamStatus = null;
  EC.editingExamRejectionReason = '';
...
/* Shown when a direct edit is blocked because students already
   submitted answers. Lets the teacher submit their in-progress edits
   as a brand-new exam (for fresh admin approval) without losing any
   of the changes they just made. */
function ecShowDuplicateOffer(errEl){
  var existing = document.getElementById('ecDuplicateOfferBtn');
  if (existing) existing.remove();
  var btn = document.createElement('button');
  btn.id = 'ecDuplicateOfferBtn';
  btn.className = 'btn-primary btn-sm';
  btn.style.marginTop = '8px';
  btn.textContent = '📋 Duplicate as New Exam Instead';
  btn.onclick = function () {
    EC.editingExamId = null;   // next Submit click will POST as a new exam
    EC.editingExamStatus = null;
    EC.editingExamRejectionReason = '';
    ecUpdateEditModeUI();
    btn.remove();
    if (errEl) { errEl.textContent=''; errEl.style.display='none'; }
    ecToast('Ready to submit as a new exam — click "Submit to Admin for Approval" again.', 'info');
  };
  if (errEl && errEl.parentNode) errEl.parentNode.insertBefore(btn, errEl.nextSibling);
}
